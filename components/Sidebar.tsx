...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, onReset, onSave }) => {
  const menuItems = [
  ...
  return (
    <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col h-full shadow-xl">
      ...
      <div className="mt-auto p-6 space-y-3">
        ...
        <button 
          onClick={onSave}
          className="w-full bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-3 rounded-xl text-xs font-black shadow-lg shadow-emerald-900/40 transition-all flex items-center justify-center space-x-2"
        >
        ...
        <button 
          onClick={onReset}
          className="w-full text-left px-4 py-2 text-[10px] font-black text-slate-500 hover:text-rose-400 transition-colors uppercase tracking-widest"
        >
          데이터 전체 초기화
        </button>
        
        <div className="flex items-center space-x-3 p-3 bg-slate-800/40 rounded-2xl border border-slate-800">
          <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-[10px] font-black text-white shadow-inner">AD</div>
          <div>
            <p className="text-white text-[11px] font-bold">인사본부 관리자</p>
            <p className="text-slate-500 text-[9px] uppercase tracking-tighter">HR Management</p>
          </div>
        </div>
      </div>
    </aside>
